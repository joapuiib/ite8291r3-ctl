...
class ite8291r3:
    ...
    def __send_ctrl(self, *payload):
        ...
        self.channel.ctrl_transfer(
            usb.util.build_request_type(
                usb.util.CTRL_OUT, usb.util.CTRL_TYPE_CLASS, usb.util.CTRL_RECIPIENT_INTERFACE),  # bmRequestType
    ...
    def __get_ctrl(self, length):
        ...
        return self.channel.ctrl_transfer(
            usb.util.build_request_type(
                usb.util.CTRL_IN, usb.util.CTRL_TYPE_CLASS, usb.util.CTRL_RECIPIENT_INTERFACE),  # bmRequestType
    ...
    def __set_effect_impl(self, control, effect=0x00, speed=0x00, brightness=0x00, color=0x00, direction_or_reactive=0x00, save=0x00):
        self.__send_ctrl(commands.SET_EFFECT, control, effect,
                         speed, brightness, color, direction_or_reactive, save)
    ...
    def freeze(self):
        ...
        # change speed to 11 (that stops the "animation" - empirical evidence)
        effect[effect_attrs.SPEED] = 11
    ...
    def enable_user_mode(self, save=False):
        self.set_effect((51, 0, self.get_brightness(), 0, 0,
                        1 if save else 0))  # change to user mode
    ...
    def set_palette_color(self, idx, color):
        if not (1 <= idx <= 7):
            raise ValueError(
                "palette color index must be between 1 and 7 (inclusive)")
...
def get(loc=None):
    ...
    if loc:
        ...
        dev = usb.core.find(
            idVendor=VENDOR_ID, custom_match=lambda d: d.idProduct in PRODUCT_IDS and d.bcdDevice == REV_NUMBER)
    ...
    fd_out = usb.util.find_descriptor(cfg[(1, 0)], custom_match=lambda e: usb.util.endpoint_direction(
